...
sealed trait Rx[+A] { self =>
  ...
  /** Dynamically switch between different `Rx`s according to the given function, applied on each element of this `Rx`. Each switch will
    * cancel the subscriptions for the previous outgoing `Rx` and start a new subscription on the next `Rx`.
    *
    * Together with `Rx#map` and `Rx.apply`, flatMap forms a `Monad`.
  ...
  /** Create the Cartesian product of two `Rx`. The output tuple contains the latest values from each input `Rx`, which updates whenever the
    * value from either input `Rx` update. This method is faster than combining `Rx`s using `for { a <- ra; b <- rb } yield (a, b)`.
    *
    * ```
    * val r1: Rx[Int]
    * val r2: Rx[Int]
  ...
  /** Merge two `Rx` into one. Updates coming from either of the incoming `Rx` trigger updates in the outgoing `Rx`. Upon creation, the
    * outgoing `Rx` first receives the current value from this `Rx`, then from the other `Rx`.
    *
    * ```
    * val r1: Rx[Int]
    * val r2: Rx[Int]
    ...
    * ```
    *
    * With this operation, `Rx` forms a `Semigroup`.
    * [Proof](https://github.com/OlivierBlanvillain/monadic-html/blob/master/monadic-rx-cats/src/main/scala/mhtml/cats.scala). `|+|` syntax
    * is available via the `monadic-rx-cats` package.
    */
  def merge[B >: A](other: Rx[B]): Rx[B] = Merge[A, B](this, other)

  /** Produces a `Rx` containing cumulative results of applying a binary operator to each element of this `Rx`, starting from a `seed` and
    * the current value of the upstream `Rx`, and moving forward in time; no internal state is maintained.
    *
    * ```
    * val numbers: Rx[Int]
  ...
  /** Returns a new `Rx` with updates where `f` is defined and mapped by `f`. If the first update is dropped, the default value is used
    * instead.
    */
  def collect[B](f: PartialFunction[A, B])(b: B): Rx[B] = Collect[A, B](this, f, b)

  /** Sample this `Rx` using another `Rx`: every time an event occurs on the second `Rx` the output updates with the latest value of this
    * `Rx`.
    *
    * ```
    * val r1: Rx[Char]
...
case class RxImpureOps[+A](self: Rx[A]) extends AnyVal {
  ...
  /** Applies the side effecting function `f` to each element of this `Rx`. Returns an `Cancelable` which can be used to cancel the
    * subscription. Omitting to canceling subscription can lead to memory leaks.
    *
    * If you use this in your code, you are probably doing it wrong.
    */
  def run(effect: A => Unit): Cancelable = Rx.run(self)(effect)

  /** Memoizes this `Rx` using an internal `Var`. This is only useful for optimizing an Rx graph, so that values generated by this `Rx` are
    * computed only once and shared between all executions.
    */
...
object Rx {
  ...
  /** The `impure.run` interpreter. Traverses the `Rx` tree and registers callbacks to run the outer most effect according to documented
    * semantics.
    */
  def run[A](rx: Rx[A])(effect: A => Unit): Cancelable = rx match {
    ...
    case Zip(self, other) =>
      ...
      var v2: Any = null
      val c1 = run(self) { a =>
        v1 = a; if (go) effect((v1, v2))
      }
      val c2 = run(other) { b =>
        v2 = b; if (go) effect((v1, v2))
      }
      go = true
      effect((v1, v2))
      Cancelable { () =>
    ...
    case im: Imitate[A] =>
      ...
      if (!self.imitating) {
        ...
        Cancelable { () =>
          cc.cancel; self.imitating = false
        }
...
class Var[A](initialValue: Option[A], register: Var[A] => Cancelable) extends Rx[A] {
  ...
  /** Is there anything currently subscribed to this `Var`?
    *
    * This method is intended to be used to test the absence of memory leak. For instance, all `Var`s should be cold after canceling a
    * `mount`.
    */
  def isCold: Boolean = subscribers.isEmpty

  /** Updates this `Var` with values emitted by the `other` `Rx`. This method is side effect free. Consequently, the returned `Rx` must be
    * used at least once for the imitation to take place. This `Var` the `other` `Rx` and the returned `Rx` will all emit the same values.
    *
...
object Var {
  ...
  /** Create a `Var` from an initial value and a register function .
    *
    * The register function is called as soon as this Var becomes active. The returned `Cancelable` is called when the Var becomes inactive.
    *
    * Registration & cancellation might append as many time as the Var goes active/inactive. This mechanism is what prevents `flatMap` from
    * leaking memory: make sure that everything created in the register is canceled.
    */
